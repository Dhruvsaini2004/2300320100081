...
function validateParams(stack, level, pkg, message) {
    ...
    if (stack === 'backend' && !BACKEND_PACKAGES.includes(pkg) && !SHARED_PACKAGES.includes(pkg)) {
        throw new Error(`Package "${pkg}" is not valid for stack "backend"`);
    }
    if (stack === 'frontend' && !FRONTEND_PACKAGES.includes(pkg) && !SHARED_PACKAGES.includes(pkg)) {
